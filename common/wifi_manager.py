...
class WifiManager:
    ...
    def start_webserver(self):
        print('Starting the configuration portal...')
        self.__WebServer        
    ...
    def __ReadProfiles(self):
        ...
        for line in lines:
            ...
            if len(profile)==3:
                default=1
            profiles[profile[0]] = (profile[1],default)
        return profiles
        ...
        self.client.sendall("""
            ...
            <html lang="en">
                ...
                <body style="background-color:#1c87c9;">
                    ...
                    <h3 style="text-align:center;">SSID:</h3>
        """.format(self.ap_ssid))
        for ssid, *_ in self.wlan_sta.scan():
            ssid = ssid.decode("utf-8")
            if ssid=="":continue
            self.client.sendall("""
                        <p style="text-align:center;"><input type="radio" name="ssid" value="{0}" id="{0}"><label for="{0}">&nbsp;{0}</label></p>
            """.format(ssid))
        self.client.sendall("""
